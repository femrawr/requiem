...
func (*MacroCommand) Exec(ses *discordgo.Session, msg *discordgo.MessageCreate, args []string) {
	...
	if utils.HasFlag(content, "register") {
		...
		macros[name] = parsed.Encode()

		ses.ChannelMessageSendReply(msg.ChannelID, fmt.Sprintf("🟩 Successfully registerd macro \"%s\"", name), msg.Reference())
		return
	}

	theMacro, exists := macros[args[0]]
	if !exists {
